		     ...
		     int lineno)
{
	...
	PyDict_SetStrictLookup(u->u_consts);
	u->u_names = PyDict_New();
	if (!u->u_names) {
		compiler_unit_free(u);
		return 0;
	}
	PyDict_SetStrictLookup(u->u_names);
...
compiler_add_o(struct compiler *c, PyObject *dict, PyObject *o)
{
	...
	hash = PyDict_GetHashFromKey(t);
	if (hash == -1)
		return -1;
	v = PyDict_GetItemWithHash(dict, t, hash);
	if (!v) {
		...
		if (PyDict_SetItemWithHash(dict, t, v, hash) < 0) {
			Py_DECREF(t);
			Py_DECREF(v);
			return -1;
		}
...
{
	basicblock *start, *cleanup, *end, *setup_block;
	int setup_index;
...
{
	basicblock *loop, *orelse, *end, *anchor = NULL, *setup_block;
...
compiler_visit_expr(struct compiler *c, expr_ty e)
{
	...
	*/
	...
	case Const_kind: {
		...
		enum _expr_const constant = e->v.Const.constant;
