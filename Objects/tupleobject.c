...
static long
tuple_relaxedhash(PyTupleObject *v)
{
	register long x, y;
	register Py_ssize_t len = Py_SIZE(v);
...
static int
tuple_equal(PyTupleObject *vt, PyTupleObject *wt)
{
	register PyObject **p, **q;
	PyObject *a, *b;
	struct _typeobject *t;
	Py_ssize_t len = Py_SIZE(vt);
	if (len != Py_SIZE(wt))
		return 0;
	p = vt->ob_item;
	q = wt->ob_item;
...
PyTypeObject PyTuple_Type = {
	...
	0,					/* tp_del */
	0,					/* tp_version_tag */
	(hashfunc)tuple_relaxedhash,					/* tp_relaxedhash */
	(cmpfunc)tuple_equal,					/* tp_equal */
};
