...
PyObject *
PyDict_DeepCopy(register PyObject *a)
{
	PyDictObject *mp;
...
void
PyDict_SetStrictLookup(PyObject *dp) {
	if (dp)
...
*/
long
PyDict_GetHashFromKey(PyObject *key)
{
	long hash;
	if (!PyString_CheckExact(key) ||
	    (hash = ((PyStringObject *) key)->ob_shash) == -1)
...
/* It's the same of PyDict_GetItem, but uses a precalculated hash value.
   Also, there's no need to check if op is a dictionary, because it's for sure.
   Used only on compiler.c for dictionaries holding consts, names, etc.
*/
PyObject *
PyDict_GetItemWithHash(PyObject *op, PyObject *key, long hash)
{
	PyDictObject *mp = (PyDictObject *)op;
	PyDictEntry *ep;
	PyThreadState *tstate;
...
int
PyDict_SetItemWithHash(register PyObject *op, PyObject *key, PyObject *value, long hash)
{
...
static long
dict_relaxedhash(PyDictObject *v)
{
...
static int
dict_strict_equal(PyDictObject *vd, PyDictObject *wd)
{
	PyDictEntry *p;
	register Py_ssize_t len;
	...
	while (--len >= 0) {
		...
		if (v1 != NULL) {
			...
			long hash;
			k = entry->me_key;
			hash = PyDict_GetHashFromKey(k);
			if (hash == -1)
				return -1;
			v2 = PyDict_GetItemWithHash((PyObject *) wd, k, hash);
			if (!v2)
				return 0;
			t = v1->ob_type;
			if (t != v2->ob_type)
				...
				register int cmp = t->tp_equal(v1, v2);
				if (cmp <= 0)
					return cmp;
			}
		}
...
PyTypeObject PyDict_Type = {
	...
	0,					/* tp_del */
	0,					/* tp_version_tag */
	(hashfunc)dict_relaxedhash,					/* tp_relaxedhash */
	(cmpfunc)dict_strict_equal,					/* tp_equal */
};
