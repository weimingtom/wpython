...
static long
complex_relaxedhash(PyComplexObject *v)
{
	register unsigned char *p;
	register int hash = complex_hash(v);
	if (hash == -1)
		return -1;
	p = (unsigned char *) &v->cval.real;
	/* distinguish 0.0 from -0.0 on IEEE platforms */
	hash += (v->cval.real == 0.0) &
			(*p == 0) & (p[sizeof(double) - 1] == 0);
	p = (unsigned char *) &v->cval.imag;
	/* distinguish 0.0 from -0.0 on IEEE platforms */
	hash += ((v->cval.imag == 0.0) & (*p == 0) &
			(p[sizeof(double) - 1] == 0)) << 1;
	return hash == -1 ? -2 : hash;
}

static int
complex_equal(PyComplexObject *a, PyComplexObject *b)
{
...
PyTypeObject PyComplex_Type = {
	...
	0,					/* tp_del */
	0,					/* tp_version_tag */
	(hashfunc)complex_relaxedhash,					/* tp_relaxedhash */
	(cmpfunc)complex_equal,					/* tp_equal */
};
