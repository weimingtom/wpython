...
PyObject *
PyList_DeepCopy(register PyObject *a)
{
	PyListObject *np;
	PyObject **src, **dest;
...
static long
list_relaxedhash(PyListObject *v)
{
	register long x, y;
	register Py_ssize_t len = Py_SIZE(v);
	register PyObject **p, *o;
	long mult = 1000003L;
	x = 0x345678L;
	p = v->ob_item;
	while (--len >= 0) {
...
static int
list_equal(PyListObject *vl, PyListObject *wl)
{
	...
	while (--len >= 0) {
		...
		if (t != b->ob_type)
			...
			register int result = t->tp_equal(a, b);
			if (result <= 0)
				return result;
		}
	}
...
PyTypeObject PyList_Type = {
	...
	0,					/* tp_del */
	0,					/* tp_version_tag */
	(hashfunc)list_relaxedhash,					/* tp_relaxedhash */
	(cmpfunc)list_equal,					/* tp_equal */
};
