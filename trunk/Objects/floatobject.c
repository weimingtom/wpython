...
static long
float_relaxedhash(PyFloatObject *v)
{
	register unsigned char *p;
	register int hash = _Py_HashDouble(v->ob_fval);
	if (hash == -1)
		return -1;
	p = (unsigned char *) &v->ob_fval;
	/* distinguish 0.0 from -0.0 on IEEE platforms */
	hash += (v->ob_fval == 0.0) & (*p == 0) & (p[sizeof(double) - 1] == 0);
	return hash == -1 ? -2 : hash;
}

static int
float_equal(PyFloatObject *a, PyFloatObject *b)
{
...
PyTypeObject PyFloat_Type = {
	...
	0,					/* tp_del */
	0,					/* tp_version_tag */
	(hashfunc)float_relaxedhash,					/* tp_relaxedhash */
	(cmpfunc)float_equal,					/* tp_equal */
};
